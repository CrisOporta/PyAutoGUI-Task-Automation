"""
MACRO RECORDER - Grabador y Reproductor de Macros
Versión Completa con captura total de teclas incluyendo Windows
"""
...
import json
import os
import keyboard  # Para captura completa de teclas Windows
...
class MacroRecorder:
    """Grabador de macros con captura completa de todas las teclas"""
    ...
    def _keyboard_callback(self, event):
        """Callback para eventos de teclado - captura TODO incluyendo Windows"""
        if not self.recording:
            return
        
        # Solo procesar eventos 'down'
        if event.event_type != 'down':
            return
        
        # Evitar duplicados (50ms debounce)
        current_time = time.time()
        if current_time - self.last_keyboard_event_time < 0.05:
            return
        ...
        # Detectar modificadores presionados
        modifiers = []
...
def main():
    ...
    ╔═══════════════════════════════════════════════════════════╗
    ║                                                           ║
    ║           MACRO RECORDER - GRABADOR DE MACROS            ║
    ║                                                           ║
    ...
    ⚠️  TIP: Ejecuta como Administrador para mejor captura de teclas
    """)
    ...
    while True:
        ...
        try:
            if choice == '1':
                ...
                # Reproducir macro - muestra lista y permite elegir
                macros = recorder.list_macros()
